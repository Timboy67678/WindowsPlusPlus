...
LRESULT MainWindow::on_create(HWND hWnd, WPARAM wParam, LPARAM lParam) {
    ...
    m_ButtonOne = create_button(_T("Click Me!"), 0, 0, 150, 25);
    m_ButtonOne->register_command_callback(BN_CLICKED, [this](WPARAM, LPARAM) {
        static int x = 0;
        std::tstring button_counter_str = TEXT("Button Clicked: ") + std::to_tstring(++x);
        m_ButtonOne->set_text(button_counter_str);
    });

    m_CheckBoxOne = create_check_box(_T("Check Me!"), 0, 30, 150, 25);
    m_CheckBoxOne->register_command_callback(BN_CLICKED, [this](WPARAM, LPARAM) {
        m_ButtonOne->set_shield(m_CheckBoxOne->get_checked() == BST_CHECKED);
    });

    m_ComboBoxOne = create_combo_box(0, 60, 150, 25);
    m_EditTextOne = create_edit_text(0, 90, 150, 25, _T("Edit me!"));
    m_ListViewOne = create_list_view(0, 120, 350, 150);

    m_RadioButtonGroup = create_radio_button_group();
    auto radiobuttonone = m_RadioButtonGroup->create_button(_T("Radio 1"), 0, 270, 150, 25, TRUE);
    auto radiobuttontwo = m_RadioButtonGroup->create_button(_T("Radio 2"), 0, 295, 150, 25);
    auto radiobuttonthree = m_RadioButtonGroup->create_button(_T("Radio 3"), 0, 320, 150, 25);

    auto radio_grptwo = create_radio_button_group();
    auto radio_one = radio_grptwo->create_button(_T("Radio 2 1"), 0, 345, 150, 25, TRUE);
    auto radio_two = radio_grptwo->create_button(_T("Radio 2 2"), 0, 370, 150, 25);
